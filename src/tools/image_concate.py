...
def concat_image(image_path: str, image_prefix: str):
    """
    ...
    for i in range(len(img_str_list)):
        img_str_list[i] = img_str_list[i].replace(image_prefix,'')
    ...
    for x in image_collections:
        image_path_list = [image_path + image_prefix + x + '_' + y + '.jpg' for y in image_collections[x]]
...
def main():
    ...
    for image_pre in args.image_prefix:
        concat_image(args.image_path, image_pre)
